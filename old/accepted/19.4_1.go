...
func uniteArray(array1 []int, array2 []int) (array []int) {
	array = make([]int, len(array1)+len(array2), len(array1)+len(array2))
...
func sortArray(array []int) (newArray []int) {
	// нужно использовать sort.Ints() - быстрее и работает в исходном массиве,
	// но хотелось попробовать самому
...
func main() {
	outputArray9 := uniteArray(sortArray(inputArray4), sortArray(inputArray5))
