...
'L' position 9
*/
func containLetters(text []string, letters []rune, registr int) (target [][]int) {
...
func generateAbracadabra(maxSentences, maxCountWords, minLenWord, maxLenWord int) (text []string) {
	defaultNum, tempWs, tempLs := 4, 1, 1
	rand.Seed(time.Now().UnixNano())
	abc := usedLetters()
	if maxSentences < 1 {
		fmt.Printf("(Значение предложений было изменено на стандартное: %v)\n", defaultNum)
		maxSentences = defaultNum
	} else if maxSentences != 1 {
		maxSentences = rand.Intn(maxSentences-1) + 1
	}
	if maxCountWords < 1 {
		fmt.Printf("(Значение количества слов было изменено на стандартное: %v)\n", defaultNum)
		maxCountWords = defaultNum
	} else if maxCountWords == 1 {
		tempWs = 0
	}
	if maxLenWord < 2 {
		fmt.Printf("(Значение длины слов было изменено на стандартное: %v)\n", defaultNum)
		maxLenWord = defaultNum
	} else if maxLenWord == 2 {
		tempLs = 0
	}
	if minLenWord < 1 || minLenWord >= maxLenWord {
		fmt.Printf("(Значение минимальной длины слов было изменено на "+
			"стандартное: %v)\n", maxLenWord/2)
...
func generateLetters(maxLetters, addCyrillic int) (letters []rune) {
	...
	if addCyrillic != 1 && addCyrillic != 0 {
		fmt.Printf("(Значение использования кириллицы было изменено на " +
			"стандартное: 1)\n")
		addCyrillic = 1
	}
...
func main() {
	...
	// 		^ функция создания массива из случайного количества предложений не более maxSentences
	//		  со случайным количеством слов не более maxCountWords случайной длины от
	//		  minLenWord до maxLenWord
	letters := generateLetters(4, 1)
	//			^ функция создания массива случайного числа рун до maxLetters латиницей или
	//			  с добавлением кириллицы при addCyrillic=1
